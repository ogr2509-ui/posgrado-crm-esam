...
import {
  History, UserCheck, MessageSquare, Clock, MapPin, Briefcase, Mail, Phone,
  Calendar, Download, Eye, FileText, GraduationCap, User, X, ZoomIn
} from 'lucide-react';
...
export function StatusTimelineModal({
  ...
  // Modal lightbox preview state for documents
  ...
  const openDocumentPreview = (dataUrl: string | null | undefined, title: string) => {
    if (!dataUrl) return;
    ...
    try {
      if (dataUrl.startsWith('data:')) {
        ...
        // Open in built-in lightbox modal
        setPreviewDoc({ url: blobUrl, title, isPdf });
      } else {
        const isPdf = dataUrl.toLowerCase().endsWith('.pdf');
        setPreviewDoc({ url: dataUrl, title, isPdf });
      }
    } catch (e) {
      console.error('Error opening document:', e);
      setPreviewDoc({ url: dataUrl, title, isPdf: false });
    }
  };
...
=====================================================`;
  ...
  return (
    <>
                ...
                {registration.ciAnversoUrl ? (
                  <div className="space-y-3">
                    {registration.ciAnversoUrl.startsWith('data:image') ? (
                      <div
                        onClick={() => openDocumentPreview(registration.ciAnversoUrl, `C.I. Anverso - ${registration.fullName}`)}
                        className="w-full h-36 rounded-lg overflow-hidden border border-slate-800 bg-slate-900 relative group cursor-pointer"
                      >
                        <img
                        ...
                        <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5 text-white text-xs font-bold">
                          <ZoomIn className="w-4 h-4 text-blue-400" /> Ampliar Documento
                        </div>
                      </div>
                    ) : (
                      <div className="p-4 rounded-lg bg-slate-900 border border-slate-800 text-center space-y-1">
                        ...
                        <p className="text-xs font-bold text-slate-300">Documento Adjunto (PDF/Word)</p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => openDocumentPreview(registration.ciAnversoUrl, `C.I. Anverso - ${registration.fullName}`)}
                        className="flex-1 py-2 px-3 rounded-lg bg-blue-600/20 hover:bg-blue-600/40 text-blue-300 font-bold text-xs border border-blue-500/30 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 py-2 px-3 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-300 font-bold text-xs border border-emerald-500/30 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                      >
                ...
                {registration.ciReversoUrl ? (
                  <div className="space-y-3">
                    {registration.ciReversoUrl.startsWith('data:image') ? (
                      <div
                        onClick={() => openDocumentPreview(registration.ciReversoUrl, `C.I. Reverso - ${registration.fullName}`)}
                        className="w-full h-36 rounded-lg overflow-hidden border border-slate-800 bg-slate-900 relative group cursor-pointer"
                      >
                        <img
      ...
      {previewDoc && (
        <div className="fixed inset-0 z-[9999] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className="max-w-4xl w-full bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            {/* Lightbox Header */}
            ...
            {/* Lightbox Footer */}
            <div className="p-3 bg-slate-950 border-t border-slate-800 flex items-center justify-end gap-3">
              <button
                onClick={() => handleDownloadDocument(previewDoc.url, `${previewDoc.title.replace(/\s+/g, '_')}.png`)}
