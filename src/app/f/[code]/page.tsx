...
export default async function PublicFormPage({ params }: { params: { code: string } }) {
  ...
  try {
    ...
    // 3. Complete Advisor and Program objects for THIS specific program
    if (link) {
    ...
    // 4. Render Registration Form ONLY for this exact matching program
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full p-8 rounded-3xl bg-slate-900 border border-slate-800 text-center space-y-4 shadow-2xl">
          ...
          <p className="text-xs text-slate-400">
            No se encontró un programa activo con el código <strong className="text-white font-mono">{rawCode}</strong>. Por favor verifica el enlace oficial.
          </p>
        </div>
      </div>
    );
